...
- [ ] **`ai_generator/views.py` → `save()`, строки ~112-114** — `gen_request = get_object_or_404(GenerationRequest, id=request.POST.get("generation_request_id"))` ищет `GenerationRequest` без проверки `gen_request.user == request.user`. IDOR: любой залогиненный пользователь может подставить чужой `generation_request_id` в hidden-поле формы и сохранить квиз по чужой AI-генерации. Стоит добавить фильтр по владельцу прямо в `get_object_or_404` (`GenerationRequest, id=..., user=request.user`) — тогда чужой `id` будет просто давать честную 404 вместо утечки данных.
